...
def test_abstain_zero_trades():
    events = [_event(4500000, "LONG", 0.5), _event(4800000, "SHORT", -0.3)]
...
def test_always_take_takes_every_candidate():
    events = [_event(4500000, "LONG", 1.0), _event(4800000, "SHORT", -2.0)]
    bars = _flat_bars(n=60)
    feats = build_event_features(events, {"TEST": bars})
...
def test_abstained_events_have_zero_net_r_in_ledger():
    events = [_event(4500000, "LONG", 5.0), _event(4800000, "LONG", -3.0)]
    bars = _flat_bars(n=60)
    calls = [0]
...
def test_ledger_aggregate_derives_from_ledger():
    events = [_event(i * 300_000 + 4500000, "LONG", 0.1 * i) for i in range(10)]
...
def test_ledger_is_immutable():
    events = [_event(4500000, "LONG", 1.0)]
    bars = _flat_bars(n=60)
    feats = build_event_features(events, {"TEST": bars})
    result = evaluate(events, feats, make_always_take_predictor(), seed=42)
...
def test_reconciliation_passes():
    events = [_event(4500000, "LONG", 0.5), _event(4800000, "LONG", -0.3)]
    bars = _flat_bars(n=60)
    feats = build_event_features(events, {"TEST": bars})
    result = evaluate(events, feats, make_always_take_predictor(), seed=42)
...
def test_deterministic_reproduction():
    events = [_event(i * 300_000 + 4500000, "LONG", 0.1 * i) for i in range(20)]
...
def test_train_test_split_is_respected():
    events = [
        ...
        _event(5100000, "LONG", 3.0, split="test"),
    ]
    bars = _flat_bars(n=60)
    feats = build_event_features(events, {"TEST": bars})
    result = evaluate(events, feats, make_always_take_predictor(), seed=42)
...
def test_invalid_split_raises():
    events = [_event(4500000, "LONG", 1.0, split="garbage")]
    bars = _flat_bars(n=60)
    feats = build_event_features(events, {"TEST": bars})
...
def test_verify_splits_fails_on_train_leak():
    events = [_event(4500000, "LONG", 1.0, split="train")]
    assert not verify_splits(events, frozen_test_start_ts=4600000)


def test_verify_splits_fails_on_test_before_boundary():
    events = [_event(4600000, "LONG", 1.0, split="test")]
    assert not verify_splits(events, frozen_test_start_ts=5000000)
...
def test_shuffled_control_does_not_touch_test():
    events = [
        _event(4500000, "LONG", 1.0, split="train"),
        _event(4800000, "LONG", -1.0, split="train"),
        _event(5100000, "LONG", 0.5, split="test"),
        _event(5400000, "LONG", -0.5, split="test"),
    ]
    bars = _flat_bars(n=50)
    feats = build_event_features(events, {"TEST": bars})
...
def test_adversarial_predictor_has_no_outcome_access():
    events = [_event(4500000, "LONG", 1.0), _event(4800000, "SHORT", -1.0)]
    bars = _flat_bars(n=60)
    feats = build_event_features(events, {"TEST": bars})
...
def test_golden_ledger_matches_hand_computation():
    events = [
        _event(4500000, "LONG", 0.5),
        _event(4800000, "LONG", -0.3),
        _event(5100000, "LONG", 0.2),
    ]
    bars = _flat_bars(n=60)
    feats = build_event_features(events, {"TEST": bars})
    result = evaluate(events, feats, make_always_take_predictor(), seed=42)
