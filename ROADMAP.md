...
The outcomes were observed and measured on the verified `okx-btc-usdt-swap-5m-1776698400000-1784474400000` snapshot.

### Empirical Observations
*   **Cost Sensitivity:** Fees and slippage are the dominant forces on short horizons. On 5m baseline, "tight" setup net_R drops from +0.0027R (zero-cost) to -0.8207R (all-taker).
*   **Aggregate Volatility/Horizon Mismatch:** Higher intervals (1h/4h) with short real-time horizons (e.g. 4h) experience very high timeout rates (up to 95.42% for 4h) due to the ATR stop/target barriers being too wide to be hit in such a short window.
*   **Base Expectancy:** Zero-cost net_R values are centered at ~0.0R across all setups, confirming a clean, unbiased baseline.
*   **Ambiguity:** Ambiguous-bar rates are very low (e.g. 0.0387% for 5m medium), validating the simulation's tie-break rules.

### Exit
The generated JSON observations were persisted to the snapshot directory:
*   `observations.json` (5m baseline): hash verified on execution box.
*   `observations_stage_b.json` (15m/1h/4h setups): hash verified on execution box.
*   All 237 test suite passes from a clean checkout on remote box.
...
## Conditional Execution Research Phase

**Status:** LOCKED

**Unlock condition:**

* one Hunter entry model and policy are frozen,
* the Hunter passes its fixed-exit frozen test,
* market, simulation, dataset, evaluation, and policy authorities are locked,
* an `ExecSpec V0` has been reviewed and frozen.

### Step 1 — trajectory readiness

* define the execution-state schema,
* pin entry, execution, and risk versions per trade,
* record append-only paper and shadow trajectories,
* continue ghost paths to the locked horizon,
* verify timestamp and causality contracts.

### Step 2 — model-free headroom

* calculate valid early-close liquidation values,
* calculate hindsight stopping upper bounds,
* report paired oracle headroom,
* issue PASS / HOLD / FAIL on whether execution research has enough
  measurable economic room.

### Step 3 — deterministic challengers

* compare deterministic `HOLD / CLOSE` policies with the fixed-exit champion,
* use the same entries and same market paths,
* retain only challengers with positive paired out-of-fold delta.

### Step 4 — learned optimal stopping

* train a shallow pooled continuation-value regressor,
* run chronological purged out-of-fold forward replay,
* evaluate paired incremental net_R,
* run cost, latency, concentration, and regime-stability audits.

### Step 5 — shadow and promotion

* freeze one execution model and policy version,
* pin policy version at trade entry,
* run shadow or paper comparison,
* promote only after the versioned execution gates pass.

### Explicitly locked

Until a measured limitation requires them:
